...
const char * const BITCOIN_CONF_FILENAME = "alpha.conf";
const char * const BITCOIN_SETTINGS_FILENAME = "settings.json";

ArgsManager gArgs;

// !SCASH
static bool fExecutableNamedAlpha;
// !SCASH END
...
std::set<std::string> ArgsManager::GetUnsuitableSectionOnlyArgs() const
{
    ...
    // !ALPHA
    if (m_network == ChainTypeToString(ChainType::ALPHAMAIN) || m_network == ChainTypeToString(ChainType::MAIN)) return std::set<std::string> {};
    // !ALPHA END
...
std::list<SectionInfo> ArgsManager::GetUnrecognizedSections() const
{
    ...
    static const std::set<std::string> available_sections{
        // !ALPHA
        ChainTypeToString(ChainType::ALPHAREGTEST),
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    fExecutableNamedAlpha = filename.find("alpha") != std::string::npos;
    // !ALPHA END
...
fs::path GetDefaultDataDir()
{
    ...
    // Windows
    // !ALPHA
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Alpha";
    // !ALPHA END
#else
    fs::path pathRet;
    ...
    // macOS
    // !ALPHA
    return pathRet / "Library/Application Support/Alpha";
    // !ALPHA END
#else
    // Unix-like
    // !ALPHA
    return pathRet / ".alpha";
    // !ALPHA END
#endif
#endif
}
...
{
    // !ALPHA
    return m_network == ChainTypeToString(ChainType::ALPHAMAIN) || m_network == ChainTypeToString(ChainType::MAIN) || m_network_only_args.count(arg) == 0;
    // !ALPHA END
}
