...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    // !ALPHA
    widget->setPlaceholderText(QObject::tr("Enter an Alpha address (e.g. %1)").arg(
    // !ALPHA END
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    // !ALPHA
    QString ret = QString("alpha:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    // !ALPHA END
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    // !ALPHA
    if (chain == ChainType::ALPHAMAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Alpha.lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Alpha (%s).lnk", ChainTypeToString(chain)));
    // !ALPHA END
}
...
fs::path static GetAutostartFilePath()
{
    ...
    // !ALPHA
    if (chain == ChainType::ALPHAMAIN)
        return GetAutostartDir() / "alpha.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("alpha-%s.desktop", ChainTypeToString(chain)));
    // !ALPHA END
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        // !ALPHA
        if (chain == ChainType::ALPHAMAIN)
            optionFile << "Name=Alpha \n";
        else
            optionFile << strprintf("Name=Alpha (%s)\n", ChainTypeToString(chain));
        // !ALPHA END
